...
const Engagement = () => {
    ...
    const fetchEngagement = async () => {
        ...
        try {
            ...
            toast.update(loadingToast, { render: "Engagement loaded", type: "success", isLoading: false, autoClose: 500 });
        } catch (error) {
            toast.update(loadingToast, { render: error.response?.data?.message || "Error fetching Engagement", type: "error", isLoading: false, autoClose: 500 });
        }
    };
    ...
    const handleSubmit = async (e) => {
        ...
        try {
            if (editId) {
                await api.put(`/engagement-types/${editId}`, formData);
                toast.success("Engagement updated successfully");
            } else {
                await api.post("/engagement-types/", formData);
            ...
            toast.error(error.response?.data?.message || "Error saving Engagement");
        } finally {
            setIsLoading(false);
        }
    };
        ...
        Swal.fire({
            ...
            if (result.isConfirmed) {
                ...
                try {
                    ...
                    toast.error(error.response?.data?.message || "Error deleting Engagement");
                } finally {
                    setLoadingId(null);
                }
            }
        ...
        <div className="flex flex-col gap-y-4">
            ...
            <Modal isOpen={isModalOpen} onClose={() => setIsModalOpen(false)}>
                <form onSubmit={handleSubmit}>
                    <label className="block mb-2 text-sm font-medium">Engagement Type</label>
                    ...
                    <button type="submit" className="mt-4 w-full bg-primary-500 text-white py-2 rounded-lg hover:bg-primary-600 transition flex justify-center items-center gap-2" disabled={isLoading}>
                        ...
                        {isLoading ? "Saving..." : editId ? "Update Engagement" : "Add Engagement"}
                    </button>
                </form>
            </Modal>
